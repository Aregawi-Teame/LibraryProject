...
public class SystemController implements ISystemController {
	...
	public void checkout(String memberID, String bookID) throws CheckoutException {
		...
		for (Book book : books.values()) {
			if (book.getIsbn().equals(bookID)) {
				if (!book.isAvailable())
					throw new CheckoutException("Book ID " + bookID + " is not available");
				else {
	...
	public void checkOverdueStatus(String bookID) throws BookStatusException {
		data = new DataAccessFacade();
		...
		(new CheckOverdueStatus()).init(arr);
	}
